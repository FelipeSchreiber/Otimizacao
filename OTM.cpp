...
long double newton(long double x1, long double x2, char busca)
{
	...
	//std::cout << H[0][0] << "," << H[0][1] << "," << H[1][0] << "," << H[1][1] << "\n";
	long double* grad = gradiente(x1, x2);
	long double t;
	//not(abs(grad[0]) < 0.00001 and abs(grad[1]) < 0.00001) and 
	while (k < 100) // Critérios utilizados: gradiente igual a zero (como não ficava igual a zero, demos uma margem de erro) e limite de interações
	{
...
long double QuaseNewton(long double x1, long double x2, char busca, char method)
{
	...
	long double** p = new long double*[2];//p_k = x_k+1 − x_k
	long double** q = new long double*[2];//q_k = ∇f ( x_k+1 ) − ∇f ( x_k )
	long double* d = new long double[2];
